...
import SVGSpriteLayouter from '../../lib/core/layouter.js';
import CSSLayouter from '../../lib/core/mode/css.js';
import DefsLayouter from '../../lib/core/mode/defs.js';
import StackLayouter from '../../lib/core/mode/stack.js';
import SymbolLayouter from '../../lib/core/mode/symbol.js';
import ViewLayouter from '../../lib/core/mode/view.js';

vi.mock('../../lib/core/mode/css.js');
vi.mock('../../lib/core/mode/defs.js');
vi.mock('../../lib/core/mode/stack.js');
vi.mock('../../lib/core/mode/symbol.js');
vi.mock('../../lib/core/mode/view.js');
...
describe('testing layout()', () => {
  ...
  it.each([
    ...
    const TEST_FN = vi.fn();
    // must be a plain function, not an arrow: the layouter calls it with
    // `new`, and arrow functions are not constructible
    const layout = layouters[mode].mockImplementation(function() {
      return {
        layout: TEST_FN
      };
    });
...
describe('testing constructor', () => {
  ...
  it('should fill up shapes with file sizes if config.example is true', () => {
    ...
    const TEST_SHAPE = {
      ...
      base: 'base',
      master: { id: 'master' },
      getDimensions: vi.fn().mockReturnValueOnce({}),
  ...
  it('should fill up shapes accordingly to spriter shapes ' +
    ...
    const TEST_MASTER_SHAPE = {
      ...
      base: 'base',
      master: { id: 'master' },
      getDimensions: vi.fn().mockReturnValueOnce(TEST_DIMENSIONS['100x100']),
    ...
    const TEST_FOURTH = {
      ...
      base: 'base',
      master: null,
      getDimensions: vi.fn().mockReturnValueOnce(TEST_DIMENSIONS['200x200']),
      config: { spacing: { padding: TEST_PADDINGS['0'] } }
    };
    const TEST_FIFTH = {
      id: '5',
    ...
    const TEST_SEVENTH = {
      ...
      base: 'base',
      master: null,
      getDimensions: vi.fn().mockReturnValueOnce(TEST_DIMENSIONS['300x300']),
      config: { spacing: { padding: TEST_PADDINGS['0'] } }
    };
    const TEST_EIGHT = {
      id: '5',
...
describe('testing defaultVariables', () => {
  ...
  describe('testing invert', () => {
    ...
    it('should revert number as a mustache function', () => {
      ...
      const TEST_NUMBER = 5;
      const MOCK_RENDER_FN = vi.fn().mockReturnValueOnce(TEST_NUMBER);
  ...
  describe('testing classname', () => {
    ...
    it('should call render as a mustache function', () => {
      ...
      const TEST_STR = 'test';
      const MOCK_RENDER_FN = vi.fn().mockReturnValueOnce(TEST_STR);
      ...
      it('should replace all last class if string includes spaces', () => {
        ...
        expect(
          layouter._commonData.classname()(
            TEST_STR,
            vi.fn().mockReturnValueOnce(TEST_STR)
          )
        ).toBe('test4');
      });
