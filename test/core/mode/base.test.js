...
import SVGSpriteBase from '../../../lib/core/mode/base.js';
...
describe('testings SVGSpriteBase', () => {
    ...
    describe('testing render', () => {
      it('should update config.render as expected', () => {
        ...
        expect(base.config.render).toStrictEqual(expect.objectContaining({
          png: {
            template: path.resolve(path.dirname(path.dirname(path.dirname(import.meta.dirname))), 'tmpl/common/sprite.png'),
          ...
          bmp: {
            ...
            template: path.resolve(path.dirname(path.dirname(path.dirname(import.meta.dirname))), 'tmpl/common/sprite.bmp')
          },
          webp: {
            dest: path.resolve(TEST_CONFIG.dest, 'here.webp'),
            template: path.resolve(path.dirname(path.dirname(path.dirname(import.meta.dirname))), 'tmpl/common/sprite.webp')
          }
    ...
    describe('if example passed', () => {
      it('should create default config if truthy value passed', () => {
        ...
        expect(base.config.example).toStrictEqual({
          template: path.resolve(path.dirname(path.dirname(path.dirname(import.meta.dirname))), path.join('tmpl', TEST_MODE_NAME, 'sprite.html')),
  ...
  describe('testing _buildCSSResources', () => {
    it('should transform passed files', async() => {
      ...
      const TEST_FILES = {};
      const testFn = vi.fn();
      vi.spyOn(mustache, 'render').mockReturnValueOnce('first').mockReturnValueOnce('second').mockReturnValueOnce('');
  ...
  describe('testing _buildHTMLExample()', () => {
    it('should add example in files', () => {
      ...
      vi.spyOn(mustache, 'render').mockReturnValueOnce('test example');
