...
vi.mock('../../../lib/core/utils/calculate-svg-dimensions');
...
describe('testing _complementDimensions()', () => {
  ...
  it('should call _determineDimensions and then _setDimensions if shape has no width and height', () => {
    ...
    const shape = new SVGShape(TEST_FILE, TEST_SPRITER);
    const cb = vi.fn();
    const testSetDimensions = vi.fn();

    shape.width = 0;
    shape.height = 0;
    vi.spyOn(shape, '_determineDimensions');
    shape._setDimensions = {
      bind: vi.fn().mockReturnValueOnce(testSetDimensions)
    };
...
describe('testing _determineDimensions()', () => {
  it('should set viewBox', () => {
    ...
    const shape = new SVGShape(TEST_FILE, TEST_SPRITER);
    const cb = vi.fn();
    vi.spyOn(shape, '_round').mockReturnValueOnce(10).mockReturnValueOnce(20);
  ...
  it('should call calculateSvgDimensions', () => {
    ...
    const cb = vi.fn();
    vi.spyOn(shape, '_round').mockReturnValueOnce(10).mockReturnValueOnce(20);
    vi.spyOn(shape, 'getSVG').mockReturnValueOnce(TEST_SVG);
    shape.width = 0;
    shape.height = 0;
...
describe('testing _setDimensions()', () => {
  it('should set viewbox and call cb', () => {
    ...
    const cb = vi.fn();
    vi.spyOn(shape, 'getViewbox').mockReturnValueOnce(null);
  ...
  it('should set attributes for all dimensions', () => {
    ...
    vi.spyOn(shape, 'getViewbox').mockReturnValueOnce(null);
    vi.spyOn(shape, 'getDimensions').mockReturnValueOnce({ height: TEST_HEIGHT, width: TEST_WIDTH });
    shape._setDimensions(vi.fn());
...
describe('testing _addPadding()', () => {
  it('should set viewbox from getViewbox() and add padding from config.spacing.padding', () => {
    ...
    vi.spyOn(shape, 'getViewbox').mockReturnValueOnce([100, 200, 300, 400]);
    vi.spyOn(shape, 'setViewbox');
    vi.spyOn(shape, 'setDimensions');
...
describe('testing _addMetadata()', () => {
  it('should set meta description', () => {
    ...
    shape.id = 'TEST_ID';
    shape.description = { setAttribute: vi.fn() };
    vi.spyOn(shape.dom.documentElement, 'setAttribute').mockImplementation();
  ...
  it('should set meta title', () => {
    ...
    shape.id = 'TEST_ID';
    shape.title = { setAttribute: vi.fn() };
    vi.spyOn(shape.dom.documentElement, 'setAttribute').mockImplementation();
  ...
  it('should remove aria-labelledby if neither title or descriptions contains in meta', () => {
    ...
    vi.spyOn(shape.dom.documentElement, 'hasAttribute').mockReturnValueOnce(true);
    vi.spyOn(shape.dom.documentElement, 'removeAttribute');
...
describe('testing complement()', () => {
  it('should call all functions and set ready state', async() => {
    ...
    const testComplementDimensions = vi.fn().mockImplementation(fn => fn(null));
    const testAddPadding = vi.fn().mockImplementation(fn => fn(null));
    const testAddMetadata = vi.fn().mockImplementation(fn => fn(null));
    const noop = vi.fn();
