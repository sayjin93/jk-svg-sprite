...
*jk-svg-sprite* comes with four public methods:
...
const SVGSpriter = SVGSpriter from 'jk-svg-sprite',
...
**Constructor** — This is the only method publicly exposed by *jk-svg-sprite*, so it's always your entry point. Use it to create an instance of the spriter and access the remaining three methods.
...
Depending on the particular mode and render configuration, quite a lot of resources might be generated during a single compilation run. To understand the way *jk-svg-sprite* returns these resources, please have a look at the following example:
...
**Accessing the intermediate SVG resources** — Sometimes you may want to access the single transformed/optimized SVG files that *jk-svg-sprite* produces in an intermediate step. Depending on the [configured shape transformations](configuration.md#shape-transformations) (e.g. SVG optimization with [SVGO](https://github.com/svg/svgo)), *jk-svg-sprite* will need some time for transforming the files, which is why accessing them must be an asynchronous task.
