...
import { isFunction, isObject, isPlainObject, isString } from './utils/index.js';
...
/**
 * Duck-typing check for a logger instance.
 *
 * Deliberately unchanged from upstream so that a `winston` logger supplied by
 * the host application is still recognised and used as-is — only the *default*
 * logger changed. The built-in {@link Logger} satisfies the same shape.
 *
