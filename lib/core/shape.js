...
// Both of these were lazy-loaded upstream to keep them off the startup path.
// Measured warm import cost is ~110ms for csso and ~11ms for the dimension
// calculator (which pulls the native @resvg/resvg-js binary) against a ~1s
// build for 116 icons. Loading them statically keeps the consuming code
// synchronous and, unlike a `createRequire` shim, leaves them mockable.
import { minifyBlock } from 'csso';
 ...
 * CSS selector parser.
 *
 * `css-selector-parser` v3 replaced the v1 `new CssSelectorParser()` class with
 * a factory, and reshaped the AST from nested `rule` objects into a
 * `Selector { rules: Rule[] }` tree where each rule carries an `items` array.
 * The parser is created once here rather than per CSS rule as upstream did.
 *
 * @type {Function}
 */
...
/**
 * Collect every ID and class name appearing anywhere in a parsed selector.
 *
 * Walks sibling rules (comma-separated selectors), descendant/child rules via
 * `nestedRule`, and — unlike upstream's v1 traversal — selectors nested inside
 * functional pseudo-classes such as `:not(#foo)` and `:is(.bar)`.
 *
 * @param {object} selector                              Parsed selector AST.
 * @returns {{ids: Array<string>, classnames: Array<string>}}   Names found.
 */
function collectSelectorNames(selector) {
  const ids = [];
  const classnames = [];
  ...
  const visitRule = rule => {
    for (const item of rule.items) {
      switch (item.type) {
        ...
        case 'PseudoClass':
        case 'PseudoElement': {
          if (item.argument && item.argument.type === 'Selector') {
            visitSelector(item.argument);
          }
...
SVGShape.prototype._stripInlineNamespaceDeclarations = function(element, nsMap) {
  ...
  if (nsMap[''] === this.DEFAULT_SVG_NAMESPACE) {
    ...
    // @xmldom/xmldom 0.9 returns `null` for a missing attribute (per the DOM
    // spec); 0.8 returned `undefined`, so a `!== undefined` guard now passes
    // for missing attributes and throws on the property access.
...
SVGShape.prototype._initSVG = function() {
  ...
  // The value capture is lazy: a greedy (.+) swallows everything up to the
  // last quote on the line, so two <!ENTITY> declarations sharing a line were
  // parsed as one entity with a corrupted value, which then injected raw
  // markup into the document when substituted.
  ...
  this.dom = new DOMParser({
    // 0.9 renamed `errorHandler` to `onError` and dropped the `locator` option
    // (source positions are always tracked now)
    onError(level, message) {
      throw new ArgumentError(format('Invalid SVG file (%s)', String(message).split('\n').join(' ')));
    }
  }).parseFromString(this.svg.current, 'text/xml');

  // @xmldom/xmldom 0.9 returns `null` from getAttribute() for a missing
  // attribute (per the DOM spec), where 0.8 returned an empty string. These
  // three reads test `.length`, so the result is normalised back to a string.
...
SVGShape.prototype.setNamespace = function(ns) {
  ...
  if (style.length) {
    for (const style of select('//svg:style', this.dom)) {
      style.textContent = minifyBlock(this._replaceIdAndClassnameReferences(style.textContent, substIds, substClassnames, true), { restructure: false }).css;
    }
  }
...
SVGShape.prototype._replaceIdAndClassnameReferencesInCssSelectors = function(str, rules, substIds, substClassnames) {
  ...
  for (const rule of rules) {
    ...
    if (selText === undefined) {
      ...
      const found = collectSelectorNames(parseCssSelector(selText));
      const ids = substIds === null ? [] : found.ids.filter(id => `#${id}` in substIds);
      const classnames = new Set(
        substClassnames === null ? [] : found.classnames.filter(classname => `.${classname}` in substClassnames)
      );
 ...
 */
export default function(file, spriter) {
