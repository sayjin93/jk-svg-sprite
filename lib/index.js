...
import { isFunction, isObject, isPlainObject, merge, prettySize as pretty, trimStart, zipObject } from './core/utils/index.js';
import * as async from './core/utils/async.js';
import Config from './core/config.js';
import Queue from './core/queue.js';
import Layouter from './core/layouter.js';
import svgo from './core/transform/svgo.js';
import ArgumentError from './core/errors/argument-error.js';
...
function SVGSpriter(config) {
  ...
  this._shapes = [];
  // Instance state, not shared. The prototype also declares _namespacePow,
  // and upstream never re-initialised it per instance — so every spriter in
  // a process mutated the same array via unshift(), and a spriter created
  // after a larger one inherited its namespace width. That made shape ID
  // namespaces depend on unrelated earlier runs.
  this._namespacePow = [];
